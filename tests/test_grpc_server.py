...
def test_new_session_state_uses_mixed_target_map_without_scalar_collapse():
    ...
    with pytest.raises(KeyError, match="confirmed target leverage is missing"):
        state.leverage_for_target("binance", "perpetual_futures", "SOLUSDT")
    assert state.leverage_for_target("binance", "spot", "BTCUSDT") == 1.0


def test_session_state_futures_target_without_confirmed_fact_fails_closed():
    state = SessionState(environment=1)
    state.configure_risk_runtime(
        order_target_keys={
            ("binance", "perpetual_futures", "BTCUSDT"),
    ...
    with pytest.raises(KeyError, match="confirmed target leverage is missing"):
        state.leverage_for_target("binance", "perpetual_futures", "BTCUSDT")
    assert state.leverage_for_target("binance", "spot", "ETHUSDT") == 1.0
...
class _FakeContext:
    ...
    def bind_running_publication(self, session_id: str, state: SessionState) -> None:
        ...
        assert state.claim_running_publication()
...
def test_run_strategy_returns_not_found_when_portfolio_lookup_fails(monkeypatch):
    servicer = StrategyServiceServicer(
        ...
        restore_running_sessions=False,
        session_bootstrap=pb2.StrategySessionBootstrap(),
    )
...
def test_run_strategy_rejects_wallet_schema_mismatch(monkeypatch):
    servicer = StrategyServiceServicer(
        "acct:1", "order:1", {}, "127.0.0.1:9092",
        restore_running_sessions=False,
        session_bootstrap=_bootstrap_for_source(_phase3_strategy_code()),
    )
    request = SimpleNamespace(
        portfolio_id=202,
        user_id=17,
...
def test_backtest_run_restores_portfolio_wallet_state_after_finish(monkeypatch):
    ...
    assert [item["status"] for item in session_updates] == ["running"]
...
def test_backtest_final_snapshot_failure_marks_session_recoverable(monkeypatch):
    ...
    assert "strategy_end store timeout" in state.error
...
def test_run_strategy_rejects_empty_runtime_binding_before_persist(monkeypatch):
    ...
    servicer._session_bootstrap = _bootstrap_for_source(
        FakePortfolioClient().get_active_strategy(303).code
    )
...
def test_run_strategy_mode2_preflight_rejects_before_session_creation(monkeypatch):
    ...
    servicer._session_bootstrap = _bootstrap_for_source(
        FakePortfolioClient("").get_active_strategy(404).code,
        environment=1,
    )
    resp = servicer.RunStrategy(request, context)
...
def test_run_strategy_mode2_preflight_disabled_still_resolves_stream_bindings(monkeypatch):
    ...
    _install_marketdata_client(monkeypatch, FakeMarketDataClient)
    servicer._session_bootstrap = _bootstrap_for_source(
        FakePortfolioClient("").get_active_strategy(405).code,
        environment=1,
    )
    ...
    with pytest.raises(strategy_base.StrategyUserCodeFatalError) as captured:
        servicer._run_backtest_via_platform_proxy(
            ...
            wallet=PortfolioWalletRuntime(
                portfolio_id=7,
                allowed_routes={("binance", "perpetual_futures")},
                wallets={("binance", "perpetual_futures", 11): make_backtest_wallet()},
            ),
        )
...
def test_run_session_live_finalizes_strategy_end_without_direct_terminal_write(monkeypatch):
    wallet = _wallet_with_futures_slot()
    servicer = StrategyServiceServicer("acct:1", "order:1", {}, "127.0.0.1:9092", restore_running_sessions=False)
    state = SessionState(environment=1)
...
def test_futures_stop_paths_unchanged_stop_strategy_stop_only_persists_state_and_halts_runtime():
    ...
    assert updates[-1] == (session_id, "stopping", 0, "")
...
def test_stop_only_waits_for_inflight_strategy_decision_before_lifecycle_scan(monkeypatch):
    ...
    stop_wallet = _spot_stop_wallet(environment=0, portfolio_id=7071, venue_id=221)
    state.configure_stop_runtime(
        wallet=stop_wallet,
        order_client=FakeOrderClient(),
    )
...
def _build_servicer_with_faked_preflight_deps(
    *,
    ...
    prepared_targets = ()
    strategy_digest = hashlib.sha256((strategy_code or "").encode()).hexdigest()
    if strategy_code is not None:
        try:
            prepared = _prepare_strategy_code_for_test(
                "<db:regression@v1>", strategy_code
            )
        ...
        def get_portfolio_snapshot(self, _portfolio_id: int, _user_id: int, **_kwargs):
            ...
            for target in prepared_targets:
                if target.market not in {"perpetual_futures", "delivery_futures"}:
                    continue
                snapshot.venues[0].wallet.futures.risk_metadata.append(
                    portfolio_service_pb2.FuturesRiskMetadata(
                        symbol=target.symbol,
                        configured_leverage=float(target.effective_leverage),
    ...
    servicer = StrategyServiceServicer(
        ...
        session_bootstrap=pb2.StrategySessionBootstrap(
            session_id="a" * 32,
            launch_operation_id="launch-current",
            ...
            confirmed_target_facts=[
                pb2.StrategySessionTargetLeverageFact(
                    ...
                    symbol=target.symbol,
                    effective_leverage=int(target.effective_leverage),
                    leverage_source=target.leverage_source,
                    confirmed_leverage=int(target.effective_leverage),
                )
                for target in prepared_targets
...
def test_committed_bootstrap_publishes_running_with_pending_cas(monkeypatch):
    source = (
        "class MyStrategy:\n"
        '    INPUTS = [{"exchange": "binance", "market": "perpetual_futures", "symbol": "BTCUSDT", "interval": "1m"}]\n'
        "    ORDER_TARGETS = []\n"
...
def test_worker_readiness_timeout_discards_without_durable_row(monkeypatch):
    ...
    canonical_id = "b" * 32
    servicer._session_bootstrap.session_id = canonical_id
    context = _PublicationContext(canonical_id)
...
def test_late_activation_after_timeout_releases_new_subscription(monkeypatch):
    ...
    servicer._session_start_timeout_seconds = 0.02
    servicer._session_bootstrap.session_id = "d" * 32
...
def test_strategy_source_gate_cursor_failure_has_no_direct_terminal_write(monkeypatch):
    source = (
        "class MyStrategy:\n"
        '    INPUTS = [{"exchange": "binance", "market": "perpetual_futures", "symbol": "BTCUSDT", "interval": "1m"}]\n'
        "    ORDER_TARGETS = []\n"
...
def test_otel_baseexception_is_owned_by_one_session_terminal_boundary(
    ...
    assert [item["status"] for item in terminal_updates] == ["running"]
...
def test_run_strategy_stores_effective_risk_controls(monkeypatch):
    ...
    assert state.target_leverage_facts == {
        ("binance", "perpetual_futures", "BTCUSDT"): (
            6,
            "strategy_default",
            6,
...
def test_backtest_funding_expands_sorted_venues_and_updates_wallets_before_kline(monkeypatch):
    from market_data.models import MarketKline
    from strategy_service.inputs import StrategyInput
    ...
    portfolio = PortfolioWalletRuntime(
        portfolio_id=7,
        allowed_routes={("binance", "perpetual_futures")},
        wallets=route_wallets,
    )
    for venue_id in (22, 11):
        route_wallet = make_backtest_wallet(
        ...
        route_wallet.futures.venue_id = venue_id
        portfolio.wallets[("binance", "perpetual_futures", venue_id)] = route_wallet
        portfolio.funding_position_tracker.restore(venue_id, [
            FundingPositionLegFact("BTCUSDT", "BOTH", "cross", "1.000000000000000001")
        ])
        ...
        def fetch_backtest_page(**_kwargs):
            return SimpleNamespace(
                klines=[MarketKline(
                    ...
                    open_time=32_400_000, close_time=32_459_999,
                    open=100.0, high=101.0, low=99.0, close=100.0,
                    volume=1.0, timestamp=32_459_999, market="futures",
                )],
                funding_facts=[fact],
                funding_coverage_complete=True,
                next_cursor_time_ms=32_400_000,
                has_more=False,
            )
    ...
    class PortfolioClient:
        @staticmethod
        def settle_backtest_funding(**kwargs):
            venue_id = kwargs["fact"].venue_id
            settlement_order.append(venue_id)
            assert kwargs["position_legs"][0].signed_qty_decimal == "1.000000000000000001"
            return portfolio_service_pb2.VenueIncomeEntry(
                income_entry_id=venue_id,
                session_id="sess-funding",
                venue_id=venue_id,
                income_type="funding_fee",
                symbol="BTCUSDT",
                asset="USDT",
                applied_amount_decimal="-0.1",
                calculation_details_json=json.dumps([{
                    "symbol": "BTCUSDT",
                    "position_side": "BOTH",
                    "margin_mode": "cross",
    ...
    class Engine:
        ...
        @staticmethod
        def running_strategy(_market_data):
            callback_balances.append([
                portfolio.wallets[("binance", "perpetual_futures", venue_id)].futures.wallet_balance
                for venue_id in (11, 22)
            ])
    ...
    servicer._run_backtest_via_platform_proxy(
        ...
        engine=Engine(),
        request=SimpleNamespace(start_time_ms=32_400_000, end_time_ms=32_460_000),
        declared_inputs=[StrategyInput("binance", "perpetual_futures", "BTCUSDT", "1m")],
        wallet=portfolio,
    )

    assert settlement_order == [11, 22]
    assert callback_balances == [[pytest.approx(99.9), pytest.approx(99.9)]]
...
def test_backtest_incomplete_funding_coverage_is_typed_only_with_open_futures_leg(
    monkeypatch,
    has_open_leg,
    ...
    if has_open_leg:
        portfolio.funding_position_tracker.restore(11, [
            FundingPositionLegFact("ETHUSDT", "BOTH", "cross", "2")
        ])
    ...
    class MarketDataClient:
        ...
        def fetch_backtest_page(**_kwargs):
            return SimpleNamespace(
                klines=[MarketKline(
                    symbol="ETHUSDT", interval="1m", open_time=1_000, close_time=1_999,
                    open=1.0, high=1.0, low=1.0, close=1.0, volume=1.0,
                    timestamp=1_999, market="futures",
                )],
                funding_facts=[],
                funding_coverage_complete=funding_coverage_complete,
                next_cursor_time_ms=1_000, has_more=False,
            )
    ...
    invoke = lambda: servicer._run_backtest_via_platform_proxy(
        ...
        state=SessionState(environment=0, user_id=17),
        engine=Engine(),
        request=SimpleNamespace(start_time_ms=1_000, end_time_ms=2_000),
        declared_inputs=[StrategyInput("binance", "perpetual_futures", "ETHUSDT", "1m")],
        wallet=portfolio,
    )

    if has_open_leg:
        with pytest.raises(BacktestFundingDataGapError) as captured:
            invoke()
        assert captured.value.symbol == "ETHUSDT"
        assert captured.value.venue_id == 11
        assert Engine.calls == 0
    else:
        invoke()
...
def test_spot_backtest_never_settles_funding(monkeypatch):
    ...
    portfolio = PortfolioWalletRuntime(
        portfolio_id=7,
        allowed_routes={("binance", "spot")},
        wallets={("binance", "spot", 11): route_wallet},
    )
    ...
    class MarketDataClient:
        ...
        def fetch_backtest_page(**_kwargs):
            return SimpleNamespace(
                klines=[MarketKline(
                    symbol="ZECUSDT", interval="1m", open_time=1_000, close_time=1_999,
                    open=1.0, high=1.0, low=1.0, close=1.0, volume=1.0,
                    timestamp=1_999, market="spot",
                )],
                funding_facts=[], funding_coverage_complete=None,
                next_cursor_time_ms=1_000, has_more=False,
            )

    class PortfolioClient:
        @staticmethod
        def settle_backtest_funding(**_kwargs):
            raise AssertionError("Spot must never settle Funding")
    ...
    servicer._run_backtest_via_platform_proxy(
        ...
        engine=Engine(),
        request=SimpleNamespace(start_time_ms=1_000, end_time_ms=2_000),
        declared_inputs=[StrategyInput("binance", "spot", "ZECUSDT", "1m")],
        wallet=portfolio,
    )
...
def test_backtest_preflight_uses_declared_exchange():
    calls = []

    class Client:
        @staticmethod
        def fetch_klines(**kwargs):
            calls.append(kwargs)
            return [object()]

    check = StrategyServiceServicer._proxy_backtest_availability(Client())

    assert check(
        grpc_server.StrategyInput("okx", "perpetual_futures", "ETHUSDT", "1m"),
        1_000,
        2_000,
    ) is True
    assert calls[0]["exchange"] == "okx"
