...
@pytest.mark.parametrize(
    ...
    [
        ("gtc-full", "FILLED", "0.01", "0", False),
        ("gtc-partial", "PARTIALLY_FILLED", "0.004", "0.006", True),
        ("ioc-partial", "EXPIRED", "0.004", "0", False),
        ("fok-full", "FILLED", "0.01", "0", False),
        ("fok-zero", "EXPIRED", "0", "0", False),
    ],
    ids=["gtc-full", "gtc-partial", "ioc-partial", "fok-full", "fok-zero"],
)
def test_spot_time_in_force_wallet_delta_matrix(
    case: str,
    status: str,
    fill_qty: str,
    remaining_qty: str,
    ...
    wallet.apply_order_update(update, spot_metadata())
    ...
    assert wallet.assets.get("BTC") is None or wallet.assets["BTC"].free == executed
