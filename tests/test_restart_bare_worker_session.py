...
def test_choose_runtime_id_prefers_explicit_then_state_then_session() -> None:
	...
	assert mod.choose_runtime_id("rt-explicit", {"RUNTIME_RUNTIME_ID": "rt-state"}, {"runtime_id": "rt-session"}) == "rt-explicit"
	assert mod.choose_runtime_id("", {"RUNTIME_RUNTIME_ID": "rt-state"}, {"runtime_id": "rt-session"}) == "rt-state"
	assert mod.choose_runtime_id("", {}, {"runtime_id": "rt-session"}) == "rt-session"


def test_choose_control_url_prefers_explicit_then_state_url_then_state_addr() -> None:
	mod = _load_script()

	assert mod.choose_control_url("http://127.0.0.1:5707", {"RUNTIME_AGENT_CONTROL_URL": "http://127.0.0.1:5706"}) == "http://127.0.0.1:5707"
	assert mod.choose_control_url("", {"RUNTIME_AGENT_CONTROL_URL": "http://127.0.0.1:5706"}) == "http://127.0.0.1:5706"
	assert mod.choose_control_url("", {"RUNTIME_AGENT_CONTROL_ADDR": "127.0.0.1:5706"}) == "http://127.0.0.1:5706"


def test_build_restart_payload_omits_empty_optional_values() -> None:
	mod = _load_script()

	payload = mod.build_restart_payload(session_id="", max_loss_close_pct=0.0, leverage=3.0)
	assert payload == {
		"leverage": 3.0,
	}
