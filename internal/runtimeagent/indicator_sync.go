...
func (m *IndicatorSyncManager) FlushSession(ctx context.Context, sessionID string, sealOpen bool) error {
	...
	flushesV1, requestV1 := snapshotIndicatorFlush(sessionID, state)
	flushesV2, requestV2 := snapshotIndicatorFlushV2(sessionID, state)
	state.mu.Unlock()

	if len(requestV1.GetDefinitions()) > 0 || len(requestV1.GetChunks()) > 0 {
		...
		state.mu.Lock()
		for _, flush := range flushesV1 {
			if flush.definition != nil &&
				flush.series.definitionGeneration ==
					flush.definitionGeneration {
