...
func TestIndicatorSyncManagerDoesNotDropImmediateFlushWhenWakeQueueSaturates(t *testing.T) {
	const sessions = 300
	platform := &indicatorSyncPlatform{started: make(chan struct{}, sessions)}
	manager := newIndicatorSyncManager(platform, 1)
	for index := 0; index < sessions; index++ {
		sessionID := "saturated-" + strconv.Itoa(index)
		if err := manager.ReceiveFrame(indicatorSyncFrame(sessionID, 1000)); err != nil {
	...
	for count := 0; count < sessions; count++ {
		select {
		case <-platform.started:
		case <-deadline:
			requests, _ := platform.snapshot()
			t.Fatalf("immediate flushes = %d, want %d", len(requests), sessions)
		}
	}
	requests, _ := platform.snapshot()
	if len(requests) != sessions {
		t.Fatalf("platform requests = %d, want %d", len(requests), sessions)
	}
}
